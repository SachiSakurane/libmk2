...
namespace mk2 { namespace simd { namespace ipp {
    ...
    public:
        fft(int order) : order_(order), fft_spec_(0)
        {
            ...
            function::fft_get_size<spec_type>
            ...
            function::fft_init(&fft_spec_, order_, IPP_FFT_NODIV_BY_ANY, pfft_spec_buf_, pfft_init_buf);
        ...
        {
            return function::fft_fwd_ctoc(src_re, src_im, dst_re, dst_im, fft_spec_, pfft_work_buf_);
        }
    
        IppStatus inplace(Type* src_dst_re, Type* src_dst_im)
        {
            return function::fft_fwd_ctoc_inplace(src_dst_re, src_dst_im, fft_spec_, pfft_work_buf_);
        }
        
        IppStatus inverse(const Type* src_re, const Type* src_im, Type* dst_re, Type* dst_im)
        {
            return function::fft_inv_ctoc(src_re, src_im, dst_re, dst_im, fft_spec_, pfft_work_buf_);
        }
        
        IppStatus inverse_inplace(Type* src_dst_re, Type* src_dst_im)
        {
            return function::fft_inv_ctoc_inplace(src_dst_re, src_dst_im, fft_spec_, pfft_work_buf_);
        }
